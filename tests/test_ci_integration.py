...
async def test_ci_integration(page):
    ...
    async def load_page():
        try:
            await page.goto("https://the-internet.herokuapp.com/", timeout=120000)  # Increased timeout
            await page.wait_for_selector("h1", timeout=60000)  # Wait for specific element
            print("Page loaded successfully.")
            return True
        except Exception as e:
            print(f"Error loading page: {e}")
            await page.screenshot(path="logs/failed_load.png")
            return False
